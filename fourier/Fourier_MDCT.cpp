...
namespace Fourier {
	void MDCT(float BufOut[], const float BufIn[], float BufLap[], float BufTmp[], size_t N) {
		...
		for(size_t i=0;i<N/2;i++) BufOut[N/2+i] = BufLap[i];
	...
	void IMDCT(float BufOut[], const float BufIn[], float BufLap[], float BufTmp[], size_t N) {
		...
		//! Undo transform
		for(size_t i=0;i<N;i++) BufTmp[i] = BufIn[i];
		DCT4(BufTmp, BufOut, N);
		...
		for(size_t i=0;i<N/2;i++) BufLap[i] = BufTmp[i];
	}
}
