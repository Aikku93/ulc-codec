...
| ```8h,1h..Dh```          | Zeros run (short)  | Insert zeros                                    |
| ```8h,Eh,Zh,Yh,Xh```     | Noise-fill         | Insert noise                                    |
...
To unpack: ```n = [second nybble] + 2```. This allows 3..15 zeros to be coded with two nybbles.

#### ```8h,Eh,Zh,Yh,Xh```: Noise-fill
