...
| ```0h,Zh,Yh,Xh```        | Noise-fill         | Insert noise                                    |
...
To unpack: ```n = [second nybble]```. This allows 1..14 zeros to be coded with two nybbles.
...
Because a short run can already code up to 28 zeros at once in the 4 nybbles this command uses, a long run codes 29..284 zeros at once.

To unpack: ```n = (Y<<4 | X) + 29```


#### ```0h,Zh,Yh,Xh```: Noise-fill
...
To unpack: ```n = (Z<<4 | Y) + 31``` and ```Level = (X+1)^2 * Quantizer / 8```. Then store ```n``` coefficients generated by random noise in the range [-1.0,+1.0], multiplied by ```Level```.
