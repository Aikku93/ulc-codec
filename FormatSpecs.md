...
For each of the remaining coefficients in the block, noise is used in place of encoded coefficients as follows:
    ...
    Coef[n++] = [random value] * Amplitude
    Amplitude = Amplitude * Decay

where ```[random value]``` is a randomly-generated value between -1.0 and +1.0.

Note that the quantizer here is scaled by 1/8. This is because Z contains a 4bit amplitude, compared to the 3bit amplitudes for normal coefficients, giving a larger dynamic range.
...
#### ```0h,Zh,Yh,Xh``` (X.bit[1..3] == 0): Zeros run (long)
...
Because a short run can already code up to 30 zeros at once in the 4 nybbles this command uses, a long run codes 31..542 zeros at once.

To unpack: ```n = (Z<<5 | Y<<1 | (X&1)) + 31```

#### ```0h,Zh,Yh,Xh``` (X.bit[1..3] != 0): Noise-fill
...
To unpack: ```n = (Z<<5 | Y<<1 | (X&1)) + 16``` and ```Level = (X>>1)^2 * Quantizer/2```. Then store ```n``` coefficients generated by random noise in the range [-1.0,+1.0], multiplied by ```Level```.
