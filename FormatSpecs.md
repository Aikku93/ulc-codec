...
Note that the quantizer here is scaled by 1/4. This is because Z contains a 4bit amplitude, compared to the 3bit amplitudes for normal coefficients, giving a larger dynamic range.
...
To unpack: ```n = (Z<<5 | Y<<1 | (X&1)) + 16``` and ```Level = (X>>1)^2 * Quantizer```. Then store ```n``` coefficients generated by random noise in the range [-1.0,+1.0], multiplied by ```Level```.
